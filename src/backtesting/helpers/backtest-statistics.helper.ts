...
export function calculateBacktestStatistics(
  ...
  const getTotalR = (tradesToCalculate: BacktestTrade[]): number =>
    tradesToCalculate.reduce((sum, trade) => sum + (trade.resultR ?? 0), 0);

  const getAdxBucket = (
    trade: BacktestTrade,
  ): "below25" | "25to30" | "30to35" | "35to40" | "above40" => {
    if (trade.adx < 25) {
      return "below25";
    }
  ...
  const getBucketWins = (bucket: BacktestTrade[]): number =>
    bucket.filter((trade) => trade.result === "WIN").length;

  const winMaeValues = [...buyWins, ...sellWins].map((trade) => trade.maeR);

  const winMfeValues = [...buyWins, ...sellWins].map((trade) => trade.mfeR);

  const winDurationValues = [...buyWins, ...sellWins].map(
    (trade) => trade.durationCandles,
  );

  const lossMaeValues = [...buyLosses, ...sellLosses].map(
    (trade) => trade.maeR,
  );

  const lossMfeValues = [...buyLosses, ...sellLosses].map(
    (trade) => trade.mfeR,
  );

  const lossDurationValues = [...buyLosses, ...sellLosses].map(
    (trade) => trade.durationCandles,
  );

  const lossesWithMfeAtLeast1R = [...buyLosses, ...sellLosses].filter(
    (trade) => trade.mfeR >= 1,
  ).length;

  const lossesWithMfeAtLeast2R = [...buyLosses, ...sellLosses].filter(
    (trade) => trade.mfeR >= 2,
  ).length;
